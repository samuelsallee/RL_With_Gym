# Working Double Deep Q Network to be included in other files
...
class Agent:
    def __init__(self, input_size, output_size, lr=1e-3, memory_size=250_000):
    ...
    def replay(self, batch_size, gamma):
        ...
        for i in range(batch_size):
            ...
            if done:
                ...
                target = reward + gamma * torch.max(self.target_model(next_state))
            y = self.model(state)
            y[action] = target
            self.train(state, y)
    ...
    def get_action(self, x, epsilon):
        with torch.no_grad():
            if np.random.rand() <= epsilon:
                ...
                return torch.argmax(self.model(x)).detach().numpy().item()
